...
res_medium = molecule.filter(molecule_properties__full_mwt__range=[351, 450], molecule_properties__qed_weighted__gt=0.7).only(['molecule_chembl_id', 'molecule_structures'])[:700]

# 2. Combine results into a single list
# We convert the QuerySets to lists to force execution and merge them
all_results = list(res_medium)
print(f"Fetched {len(all_results)} medium molecules.")
...
df.to_csv('../data/medium_molecules.csv', index=False)